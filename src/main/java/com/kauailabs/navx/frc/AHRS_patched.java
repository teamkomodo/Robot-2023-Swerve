/*--------------------------------------------------------*/
/*| AHRS_patched is a modification to the original NavX   |*/
/*| library made by FRC4293 to fix its compatibility with |*/
/*| WPILib 2023 in simulation.                            |*/
...
public class AHRS_patched {
     ...
     */
    public AHRS_patched(SPI.Port spi_port_id, byte update_rate_hz) {
        commonInit(update_rate_hz);
        if (m_simDevice != null) {
            io = new SimIO_patched(update_rate_hz, io_complete_sink, m_simDevice);
        } else {
    ...
    public AHRS_patched(SPI.Port spi_port_id, int spi_bitrate, byte update_rate_hz) {
     ...
     * @param update_rate_hz Custom Update Rate (Hz)
     */
    public AHRS_patched(I2C.Port i2c_port_id, byte update_rate_hz) {
     ...
     * @param update_rate_hz Custom Update Rate (Hz)
     */
    public AHRS_patched(SerialPort.Port serial_port_id, SerialDataType data_type, byte update_rate_hz) {
     ...
     * @param spi_port_id SPI port to use.
     */
    public AHRS_patched(SPI.Port spi_port_id) {
     ...
     * @param i2c_port_id I2C port to use
     */
    public AHRS_patched(I2C.Port i2c_port_id) {
     ...
     * @param serial_port_id SerialPort to use
     */
    public AHRS_patched(SerialPort.Port serial_port_id) {
    ...
    public double getRate() {
        if (m_simDevice != null) {
            SimIO_patched simio = (SimIO_patched) io;
            if (simio != null) {
    ...
    class IOCompleteNotification implements IIOCompleteNotification {
        ...
        public void setYawPitchRoll(YPRUpdate ypr_update, long sensor_timestamp) {
            AHRS_patched.this.yaw = ypr_update.yaw;
            AHRS_patched.this.pitch = ypr_update.pitch;
            AHRS_patched.this.roll = ypr_update.roll;
            AHRS_patched.this.compass_heading = ypr_update.compass_heading;
            AHRS_patched.this.last_sensor_timestamp = sensor_timestamp;
        }
        ...
        public void setAHRSPosData(AHRSPosUpdate ahrs_update, long sensor_timestamp) {
            ...
            AHRS_patched.this.yaw = ahrs_update.yaw;
            AHRS_patched.this.pitch = ahrs_update.pitch;
            AHRS_patched.this.roll = ahrs_update.roll;
            AHRS_patched.this.compass_heading = ahrs_update.compass_heading;
            yaw_offset_tracker.updateHistory(ahrs_update.yaw);

            /* Update AHRS class variables */

            // 9-axis data
            AHRS_patched.this.fused_heading = ahrs_update.fused_heading;

            // Gravity-corrected linear acceleration (world-frame)
            AHRS_patched.this.world_linear_accel_x = ahrs_update.linear_accel_x;
            AHRS_patched.this.world_linear_accel_y = ahrs_update.linear_accel_y;
            AHRS_patched.this.world_linear_accel_z = ahrs_update.linear_accel_z;

            // Gyro/Accelerometer Die Temperature
            AHRS_patched.this.mpu_temp_c = ahrs_update.mpu_temp;

            // Barometric Pressure/Altitude
            AHRS_patched.this.altitude = ahrs_update.altitude;
            AHRS_patched.this.baro_pressure = ahrs_update.barometric_pressure;

            // Status/Motion Detection
            AHRS_patched.this.is_moving = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_MOVING) != 0)
                            ? true
                            : false);
            AHRS_patched.this.is_rotating = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_YAW_STABLE) != 0)
                            ? false
                            : true);
            AHRS_patched.this.altitude_valid = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0)
                            ? true
                            : false);
            AHRS_patched.this.is_magnetometer_calibrated = (((ahrs_update.cal_status &
                    AHRSProtocol.NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0)
                            ? true
                            : false);
            AHRS_patched.this.magnetic_disturbance = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0)
                            ? true
                            : false);

            AHRS_patched.this.quaternionW = ahrs_update.quat_w;
            AHRS_patched.this.quaternionX = ahrs_update.quat_x;
            AHRS_patched.this.quaternionY = ahrs_update.quat_y;
            AHRS_patched.this.quaternionZ = ahrs_update.quat_z;
        ...
        public void setRawData(AHRSProtocol.GyroUpdate raw_data_update, long sensor_timestamp) {
            AHRS_patched.this.raw_gyro_x = raw_data_update.gyro_x;
            AHRS_patched.this.raw_gyro_y = raw_data_update.gyro_y;
            AHRS_patched.this.raw_gyro_z = raw_data_update.gyro_z;
            AHRS_patched.this.raw_accel_x = raw_data_update.accel_x;
            AHRS_patched.this.raw_accel_y = raw_data_update.accel_y;
            AHRS_patched.this.raw_accel_z = raw_data_update.accel_z;
            AHRS_patched.this.cal_mag_x = raw_data_update.mag_x;
            AHRS_patched.this.cal_mag_y = raw_data_update.mag_y;
            AHRS_patched.this.cal_mag_z = raw_data_update.mag_z;
            AHRS_patched.this.mpu_temp_c = raw_data_update.temp_c;
        ...
        public void setAHRSData(AHRSProtocol.AHRSUpdate ahrs_update, long sensor_timestamp) {
            ...
            // Magnetometer Data
            AHRS_patched.this.cal_mag_x = ahrs_update.cal_mag_x;
            AHRS_patched.this.cal_mag_y = ahrs_update.cal_mag_y;
            AHRS_patched.this.cal_mag_z = ahrs_update.cal_mag_z;
            ...
            updateDisplacement(AHRS_patched.this.world_linear_accel_x,
                    AHRS_patched.this.world_linear_accel_y,
                    update_rate_hz,
                    AHRS_patched.this.is_moving);
        ...
        void updateBoardStatus(byte op_status, short sensor_status, byte cal_status, byte selftest_status) {
            ...
            if (AHRS_patched.this.op_status == AHRSProtocol.NAVX_OP_STATUS_NORMAL) {
             ...
             */
            if (((AHRS_patched.this.cal_status
                    & AHRSProtocol.NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) != AHRSProtocol.NAVX_CAL_STATUS_IMU_CAL_COMPLETE)
                    &&
                    ((cal_status
                            & AHRSProtocol.NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) == AHRSProtocol.NAVX_CAL_STATUS_IMU_CAL_COMPLETE)) {
                ...
                if (poweron_init_completed || AHRS_patched.this.disconnect_startupcalibration_recovery_pending) {
                    AHRS_patched.this.disconnect_startupcalibration_recovery_pending = false;
                    AHRS_patched.this.yaw_angle_tracker.init();
                    System.out.printf("navX-Sensor Yaw angle auto-reset to 0.0 due to startup calibration.\n");
                }
            }

            AHRS_patched.this.op_status = op_status;
            AHRS_patched.this.sensor_status = sensor_status;
            AHRS_patched.this.cal_status = cal_status;
            AHRS_patched.this.selftest_status = selftest_status;
        }

        @Override
        public void yawResetComplete() {
            AHRS_patched.this.yaw_angle_tracker.reset();
            if (AHRS_patched.this.enable_boardlevel_yawreset) {
        ...
        public void disconnectDetected() {
            ...
            AHRS_patched.this.op_status = AHRSProtocol.NAVX_OP_STATUS_ERROR;
            AHRS_patched.this.sensor_status = 0; /* Clear all sensor status flags */
            /*
             * Flag the need to watch for a startup calibration completion event upon later
             * reconnect.
             */
            AHRS_patched.this.disconnect_startupcalibration_recovery_pending = true;
